...
export class BooksService {
    static getBooks: IGetAll = async (signal?: AbortSignal) =>
        $api
            .get<IBook[]>(apiEndpoint.Books, { signal })
            .then((resp: AxiosResponse<IBook[]>) => resp.data);

    static getBook: IGetSingle = async ({ id }, signal?: AbortSignal) =>
        $api
            .get<IBook>(apiEndpoint.Books, { params: { id }, signal })
            .then(resp => resp.data);

    static deleteBook: IDeleteSingle = async ({ id }, signal?: AbortSignal) =>
        $api
            .delete<IBook>(apiEndpoint.Books, { params: { id }, signal })
            .then(resp => resp.data);

    static updateBook: IUpdateSingle = async ({ id }, payload, signal?: AbortSignal) =>
        $api
            .patch<IBook>(apiEndpoint.Books, payload, { params: { id }, signal })
            .then(resp => resp.data);
